...
@EnableWebSecurity
@Order(120)
public class ThirdLoginAuthSecurityConfig extends WebSecurityConfigurerAdapter {
    @Value("${com.guns21.security.login.third:/third/login}")
    private String login;
    @Value("${com.guns21.security.logout.third:/third/logout}")
    ...
    @Resource(name = "beforeLoginFilter")
    private Filter beforeLoginFilter;

    @Autowired
    private AuthExtValidator authExtValidator;

    @Bean
    @ConditionalOnMissingBean(name = "thirdLoginAuthenticationProvider")
    public AuthenticationProvider thirdLoginAuthenticationProvider() {
    ...
    protected void configure(AuthenticationManagerBuilder auth) throws Exception {
        auth.authenticationProvider(thirdLoginAuthenticationProvider());
    }
